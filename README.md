...
Our current implementation requires the following code to be manually added into the JSON request file in the first POST which we know will return the required list_item_id value. Once we have the value we can then map it to a key and use it to format future GETS


```
"redirect_route": <The list_item_id key name in the same positions as the value would be returned from the POST >

```

These manually added redirect_route elements will at present be before the first GET which uses that list_item_id.
...
N.B Although the above example uses the same url in the redirect as the GET, it doesn't have to, we only really care that the key ({person_1_list_id}) and the value returned from the POST align, the redirect could be "/-/-/{person_1_list_id}/-/".
