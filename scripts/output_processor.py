...
def mean_requests(key, first, second):
    ...
    """
    return (
        first[key] * first['num_requests'] + second[key] * second['num_requests']
    ) / (first['num_requests'] + second['num_requests'])
...
def merge_requests(first, second):
    ...
    output['average_content_size'] = mean_requests(
        'average_content_size', first, second
    )
    ...
    with open(input_filename) as csv_requests:
        ...
        for row in reader:
            ...
            if request_key in requests:
                requests[request_key] = merge_requests(
